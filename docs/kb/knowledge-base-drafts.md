# Knowledge Base Drafts — Ethiopian Maids

Drafted 2026-07-12 for the Settings → Knowledge RAG store; updated same day
to the MARKETPLACE model (platform connects sponsors ↔ agencies ↔ independent
maids; fees/replacement/contract terms are sponsor↔agency matters governed by
GCC domestic-worker regulations — the source of truth). Format: each
`## DOC:` section is one knowledge-base document (heading = title, body =
content). Load with `load-kb-drafts.mjs` or paste into Settings → Knowledge.
Regulatory figures are current to early 2026 — re-check periodically.

---

## DOC: About Ethiopian Maids — who we are and what we do

Ethiopian Maids is a placement MARKETPLACE (platform + app) that connects
three sides: sponsor families in the Gulf (GCC) countries — the UAE, Saudi
Arabia, Kuwait, Qatar, Bahrain, and Oman — licensed recruitment agencies,
and independent household workers.

We serve ALL household staff roles: housemaids, housekeepers, nannies,
cooks, cleaners, elderly-care helpers, all-rounders, DRIVERS, and PRIVATE
NURSES. We do not handle company or commercial staffing, and we place only
in GCC countries.

Despite our name, the platform serves candidates of several nationalities
commonly employed in GCC households: primarily Ethiopian, alongside other
African candidates (such as Kenyan, Ugandan, Ghanaian, Gambian, and Sierra
Leonean) and Asian candidates (such as Filipino, Indonesian, Sri Lankan,
and Indian).

Our role is to FACILITATE: verified profiles, search and matching, video
interviews, communication, and a dispute-and-review system. The employment
contract, placement fees, and replacement terms are agreed directly
between the sponsor and the recruitment agency (or independent candidate),
governed by the destination country's domestic-worker regulations — those
regulations are the source of truth for fees, replacement, contract
duration, cancellation, sick leave, and similar terms. Ethiopian Maids is
not a party to those agreements.

There are three sides to our service:
- Sponsors (families and employers) browse verified candidates, interview
  them by video call, and hire through the agency or candidate they choose.
- Agencies list their candidates and manage their placements.
- Job seekers (independent domestic workers) register in our app, build a
  profile, and apply to job openings posted by sponsors.

This WhatsApp number is our customer-service channel. Registration, profile
creation, browsing, and job applications all happen in the official
Ethiopian Maids app — never over chat. We never ask for passport numbers,
bank details, or payments in WhatsApp messages.

Our team speaks English, Arabic, and Amharic.

## DOC: The Ethiopian Maids app — what it is and what you can do in it

The official Ethiopian Maids app is available on Google Play (search for
"Ethiopian Maids" or use the download button we send in chat). The iPhone
version is coming soon to the App Store.

For sponsors, the app lets you: create an account, browse available
candidates with photos, experience and skills, shortlist candidates, and
follow your hiring process.

For job seekers (maids), the app lets you: register and build your profile,
upload your documents (passport photo page, certificates), set your
preferred destination and salary, browse job openings posted by sponsors,
and apply directly.

Important safety note: we only ever share the app through the official
Google Play card in chat — we never send bare download links. If you receive
an Ethiopian Maids "link" from any other number, do not tap it.

Creating an account and browsing are free for both sides.

## DOC: How hiring works for sponsors — step by step

1. Tell us what you need. Which emirate/city you live in, live-in or
   live-out, the main duties (childcare, cooking, elderly care, general
   housework), when you need her to start, and any language or experience
   preferences.
2. We send you matching candidates as photo cards right here in WhatsApp —
   first name, nationality, years of experience, key skills, languages, and
   expected salary range. If nothing matches today, we can save your search
   and message you the moment a matching candidate becomes available (we
   keep watching for 30 days).
3. Video interview. When a candidate interests you, we schedule a video
   interview — you pick the time, we send you the video call link in chat
   plus reminders 1 hour and 10 minutes before the call. The candidate is
   notified through her app.
4. Decision and paperwork. After a successful interview, the recruitment
   agency handling the candidate finalizes the offer, employment contract,
   fees, and the visa/travel process with you directly, under your
   country's domestic-worker rules (see our country documents — those
   regulations are the source of truth for fees, replacement, contract
   duration, and cancellation). Our platform stays with you in chat
   throughout and records any dispute.
5. Arrival and follow-up. We stay in contact after placement — you can
   always reach us on this number.

We never share a candidate's full name, passport details, exact location,
or personal phone number before a confirmed booking.

## DOC: How it works for job seekers (maids) — step by step

1. Download the official Ethiopian Maids app from Google Play (we send the
   official download card in chat — tap the button).
2. Register and complete your profile: your experience, skills, languages,
   preferred destination country, and expected salary. Upload a clear photo
   of your passport information page in the app.
3. Browse and apply. Job openings from sponsor families in the GCC are
   posted in the app with salary, location, and duties. Apply to the ones
   that suit you.
4. Job alerts. If no suitable job is open today, tell us your destination
   and experience in chat — we will save a job alert and message you here
   as soon as a matching opening is posted (we keep watching for 30 days).
5. Interviews and placement. Sponsors may request a video interview. Our
   team guides you through the offer, contract, medical checks, and travel
   steps required by Ethiopian law and the destination country.

You must be at least 18 years old (21 for Saudi Arabia) and hold a valid
passport. Ethiopian citizens also complete the pre-departure requirements
set by Ethiopian overseas-employment law (see the Ethiopia requirements
document); candidates of other nationalities follow their own country's
overseas-employment rules.

## DOC: Placement fees — how they work on our platform

Placement fees are set by and paid to the recruitment agency handling the
placement (or agreed directly with an independent candidate). Ethiopian
Maids is a marketplace: we facilitate the connection, but we are NOT a
party to the fee agreement between sponsor and agency and are not
responsible for it.

The source of truth for what an agency may charge — and who must pay —
is the destination country's domestic-worker regulation. Two important
protections most GCC rules share: recruitment costs are the EMPLOYER'S
responsibility (they may never be charged to the worker or deducted from
her salary), and hiring must go through licensed channels (for example
MoHRE-licensed centres in the UAE, the Musaned platform in Saudi Arabia).

When comparing agency offers, ask the agency for a written breakdown of
what its fee includes. Typical items to confirm: candidate sourcing and
screening, the worker's medical fitness examination, worker-side visa
processing, the flight ticket, and any origin-country requirements
(training certificate, insurance). Also confirm what is NOT included —
the worker's monthly salary, sponsor-side residency and permit costs
(Emirates ID, medical insurance, work permit), and daily provisions for
live-in workers are normally the sponsor's own obligations under GCC law.

Ask us in chat for the current official fee reference for your country —
we will quote it. Never pay anyone in WhatsApp chat, and always get the
agency's full fee breakdown in writing before paying anything.

Registering and browsing on the platform are free for all sides.

## DOC: Replacement, refunds, and cancellations — how they work

Replacement, refund, and cancellation terms are part of the agreement
between the SPONSOR and the recruitment AGENCY (or independent candidate).
Ethiopian Maids is a marketplace — we facilitate the connection but we are
not a party to that agreement and not responsible for its terms.

The source of truth is the destination country's domestic-worker
regulation, which governs probation periods, agency replacement
obligations, contract duration, cancellation, sick leave, and end-of-
service rights. For example, in the UAE the standard contract includes a
probation period during which the licensed agency carries replacement
obligations; Saudi Arabia's Musaned contracts include defined replacement
and refund rights. See our country documents, and always confirm the
agency's replacement and refund terms IN WRITING before paying.

What our platform does when something goes wrong:
- Tell us here on WhatsApp — we record a formal dispute against the
  agency (or against a sponsor, when the complaint comes from the worker
  or agency side) and our team follows up with both parties.
- Both sides can rate and review each other on the platform: sponsors
  review agencies, and agencies review sponsors. Reviews are visible to
  future users.
- Agencies or sponsors with repeated verified complaints are flagged and
  can be removed from the platform.

Serious safety concerns are treated as urgent and escalated to a human
team member immediately, at any hour.

## DOC: Our role as a platform — disputes and reviews

Ethiopian Maids is a neutral marketplace connecting sponsors, licensed
recruitment agencies, and independent domestic workers. We are not the
employer, not the recruitment agency, and not a party to the employment
contract or fee agreement.

What we provide: verified candidate profiles, search and matching, video
interview scheduling, match alerts, customer service in chat, and a
dispute-and-review system that keeps both sides accountable.

What is between sponsor and agency (governed by the destination country's
domestic-worker regulations — the source of truth): placement fees and
what they include, replacement and refund terms, contract duration and
renewal, cancellation, probation, salary, working hours, rest days,
annual and sick leave, and end-of-service rights.

Disputes: message us here on WhatsApp. We record the dispute, our team
follows up with both parties, and the outcome is noted on the agency's or
sponsor's platform profile. Sponsors can review agencies and agencies can
review sponsors; repeat verified offenders are flagged or removed.

We never take payments in chat, and we never ask for passport numbers or
bank details in chat.

## DOC: UAE — rules for hiring a domestic worker

The UAE regulates domestic work under Federal Decree-Law No. 9 of 2022 (the
Domestic Workers Law), administered by MoHRE. The law covers all household
occupations — housemaids, nannies, cooks, cleaners, private drivers,
private nurses, gardeners, and similar roles. Key points every sponsor
should know (current to early 2026 — our team re-checks these regularly):

- Hiring must go through licensed recruitment agencies / domestic-worker
  service centres, with a standard MoHRE contract.
- The worker must be at least 18 years old.
- Working conditions guaranteed by law: one paid rest day per week; at
  least 12 hours of rest per day (including 8 consecutive hours); 30 days
  of paid annual leave per year.
- The employer provides food, suitable accommodation, and medical
  insurance, and pays salaries on time (within 10 days of the due date).
- Recruitment costs are the employer's responsibility — it is illegal to
  charge them to the worker or deduct them from her salary.
- Keeping the worker's passport is prohibited; the passport belongs to the
  worker.
- A probation period applies at the start of the contract during which the
  agency is responsible for replacement under its terms.
- The employer sponsors the worker's residence visa, Emirates ID, and
  medical fitness testing.

There is no UAE statutory minimum wage for domestic workers, but source
countries set salary floors; typical market salaries for Ethiopian
housemaids are in the 1,200–1,500 AED/month range [confirm current market
range with our team].

## DOC: Saudi Arabia — rules for hiring a domestic worker

Saudi Arabia manages all domestic-worker recruitment through the government
Musaned platform, under the Domestic Workers Regulation updated in 2023
(current to early 2026 — our team re-checks these regularly):

- Contracts must be created and registered through Musaned; recruitment
  outside Musaned is not lawful.
- The worker must be at least 21 years old for domestic work in Saudi
  Arabia.
- Working conditions under the updated regulation: daily working hours are
  capped (10 hours), with a paid weekly rest day, paid annual leave, and
  end-of-service entitlements.
- Wages are paid through the approved digital channels linked to Musaned.
- Health insurance is mandatory for new domestic-worker contracts.
- Passport confiscation is prohibited.
- Ethiopia and Saudi Arabia have a bilateral labour agreement covering
  domestic workers; recruitment must follow both countries' rules.

Exact costs, salary floors, and Musaned process details change — ask us in
chat and our team will confirm the current figures for your case.

## DOC: Kuwait, Qatar, Bahrain, Oman — domestic worker rules in brief

Current to early 2026 — our team re-checks these regularly. Ask us in chat
for the latest details for your specific country.

Kuwait (Law No. 68 of 2015 on Domestic Workers):
- Maximum 12 working hours per day with rest; one paid weekly day off;
  30 days paid annual leave; end-of-service benefit of one month's wage per
  year of service. The employer provides food, housing, and medical care.
  A ministerial minimum wage applies to domestic workers [confirm current
  amount]. Hiring only through licensed recruitment offices; passport
  confiscation is banned.

Qatar (Law No. 15 of 2017 on Domestic Workers):
- Maximum 10 working hours per day; one paid weekly rest day (24
  consecutive hours); three weeks of paid annual leave; end-of-service
  benefit of at least three weeks' wage per year. Qatar's minimum wage law
  (1,000 QAR/month basic, plus food and housing allowances if not provided
  in kind) applies to domestic workers.

Bahrain:
- Recruitment and work permits run through the LMRA. Domestic workers are
  covered by parts of the labour law (contracts, wage protection); a
  standard tripartite contract between employer, worker, and agency is
  required.

Oman:
- Domestic workers are hired through agencies licensed by the Ministry of
  Labour with an approved employment contract; the employer sponsors the
  visa and provides food, accommodation, and medical care. A weekly rest
  day and paid leave terms are set in the contract.

## DOC: Ethiopia — legal requirements for working abroad as a domestic worker

Ethiopian law (Overseas Employment Proclamation No. 923/2016, as amended by
Proclamation No. 1246/2021) protects Ethiopian citizens taking domestic
work abroad. What every candidate must know (current to early 2026):

- Overseas placement is legal only through agencies licensed by the
  Ethiopian government, or government-to-government programs. Ethiopia must
  have a labour agreement with the destination country.
- Minimum age is 18 (destination rules may be stricter — Saudi Arabia
  requires 21).
- An eighth-grade education is the minimum schooling requirement set by the
  proclamation.
- Before departure every worker must complete: an occupational competence
  certificate (COC) for domestic work, a medical fitness examination,
  pre-departure orientation training, and mandatory insurance coverage.
- The employment contract must be registered with the Ethiopian authorities
  before travel, and the worker keeps her own passport at all times.
- Workers must never pay illegal broker ("delala") fees. Report anyone
  demanding money for a job to the authorities and to us.

We guide every candidate through these steps as part of the placement
process — ask us in chat at any point.

## DOC: Interviews — how video interviews work

When a sponsor wants to meet a candidate, we arrange a video interview:

- The sponsor picks a convenient time in chat ("tomorrow 2pm", "today
  4pm"). We create the booking and send the video call link right here in
  WhatsApp.
- Reminders are sent automatically 1 hour and 10 minutes before the call.
- The candidate is notified through her Ethiopian Maids app with the same
  link.
- The call is a normal browser video call — no app installation is needed;
  tap the link on your phone at the scheduled time.
- Interviews are free and there is no limit — sponsors can interview more
  than one candidate before deciding.
- If the time no longer suits you, tell us in chat and we will reschedule.

Practical tips for sponsors: prepare your key questions (experience with
children or elderly, cooking, languages), and check the candidate's
expected salary and start date shown on her card.

## DOC: Privacy and safety — how we protect both sides

- We never ask for passport numbers, ID numbers, bank details, or payments
  in WhatsApp chat.
- Candidates' personal details are protected: full name, passport data,
  exact address, and personal phone numbers are shared only after a
  confirmed booking, never in the browsing stage.
- All registration and document uploads happen inside the official app,
  not over chat.
- We only share the app via the official Google Play card. Do not trust
  Ethiopian Maids "links" from unknown numbers.
- Workers keep their own passports at all times — in every GCC country we
  serve, passport confiscation by employers is prohibited.
- If you ever feel unsafe or mistreated — worker or sponsor — tell us here
  immediately and a human team member will take over. Serious safety
  concerns are escalated to our team as urgent, at any hour.
