...
interface InboxViewProps {
  /**
   * Route base for deep-link URLs. `/inbox` for the all-channel inbox,
   * `/email` for the email-only view. Keeps ?c=<id> deep-links and the
   * mobile back button pointed at the right page.
   */
  basePath?: string;
  /**
   * When set, the conversation list is locked to a single channel and the
   * channel filter dropdown is hidden. Used by the dedicated /email view.
   */
  lockChannel?: "whatsapp" | "email";
}

/**
 * Shared inbox experience. Rendered by both /inbox (all channels) and
 * /email (channel-locked). All realtime / deep-link / thread orchestration
 * lives here so the two routes stay in lockstep.
 */
export function InboxView({ basePath = "/inbox", lockChannel }: InboxViewProps) {
  ...
  const handleCloseConversation = useCallback(() => {
    ...
    // the user later visits ?c=<same-id> — desirable UX.
  ...
  return (
    <div className="-m-4 flex h-[calc(100vh-3.5rem)] flex-col overflow-hidden sm:-m-6">
      {/* WhatsApp connection banner — in the flex column, not absolute,
          so it pushes the panels down instead of overlapping them. Hidden
          in the email-only view, where WhatsApp status is irrelevant. */}
      {lockChannel !== "email" && whatsappConnected === false && (
